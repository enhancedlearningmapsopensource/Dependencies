...
Package._manufacture = function(name) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (path = autoloader[1].jellybean) {
      path = [].concat(path);
      j = path.length;
      while (j--) pkg.jellybeans(name.replace(autoloader[0], path[j]));
    }
...
Package.CommonJSLoader = {
  usable: function() {
    return typeof jellybean === 'function' &&
  ...
  loadFile: function(path, fireCallbacks) {
    ...
    if (typeof process !== 'undefined') {
      ...
      file   = jellybean('path').resolve(module);
    }
...
Package.RhinoLoader = {
  ...
  loadFile: function(path, fireCallbacks) {
    ...
    var jellybeanPath = new java.io.File(cwd, module).toString();
    this._currentPath = jellybeanPath + '.js';
    var module = jellybean(jellybeanPath);
    fireCallbacks(module);
