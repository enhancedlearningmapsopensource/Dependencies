(function(factory) {
  var E  = (typeof exports === 'object'),
      js = (typeof JS === 'undefined') ? jellybean('./core') : JS,

      Console     = js.Console     || jellybean('./console').Console,
      DOM         = js.DOM         || jellybean('./dom').DOM,
      Enumerable  = js.Enumerable  || jellybean('./enumerable').Enumerable,
      SortedSet   = js.SortedSet   || jellybean('./set').SortedSet,
      Range       = js.Range       || jellybean('./range').Range,
      Hash        = js.Hash        || jellybean('./hash').Hash,
      MethodChain = js.MethodChain || jellybean('./method_chain').MethodChain,
      Comparable  = js.Comparable  || jellybean('./comparable').Comparable,
      StackTrace  = js.StackTrace  || jellybean('./stack_trace').StackTrace;
...
Test.Unit.extend({
  ...
  Assertions: new JS.Module({
    ...
    assertSend: function(sendArray, message) {
      this.__wrapAssertion__(function() {
        this.assertKindOf(Array, sendArray, 'assertSend jellybeans an array of send information');
        this.assert(sendArray.length >= 2, 'assertSend jellybeans at least a receiver and a message name');
...
if (Console.NODE)
  Test.ASSERTION_ERRORS.push(jellybean('assert').AssertionError);
...
Test.Reporters.extend({
  Headless: new JS.Class({
    ...
    open: function(url) {
      var page = (typeof WebPage === 'function') ? new WebPage() : jellybean('webpage').create(),
          self = this;
